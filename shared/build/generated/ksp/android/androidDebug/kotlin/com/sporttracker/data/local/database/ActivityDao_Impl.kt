...
public class ActivityDao_Impl(
  ...
  init {
    ...
    this.__insertAdapterOfActivityEntity = object : EntityInsertAdapter<ActivityEntity>() {
      ...
      protected override fun bind(statement: SQLiteStatement, entity: ActivityEntity) {
        ...
        val _tmp: String = __converters.fromStorageType(entity.storageType)
  ...
  public override suspend fun insertActivity(activity: ActivityEntity): Unit = performSuspending(__db, false, true) { _connection ->
    __insertAdapterOfActivityEntity.insert(_connection, activity)
  }

  public override suspend fun updateActivity(activity: ActivityEntity): Unit = performSuspending(__db, false, true) { _connection ->
    __updateAdapterOfActivityEntity.handle(_connection, activity)
  }

  public override fun observeAllActivities(): Flow<List<ActivityEntity>> {
    ...
    return createFlow(__db, false, arrayOf("activities")) { _connection ->
      ...
      try {
        ...
        while (_stmt.step()) {
          ...
          val _tmpStorageType: StorageType
          val _tmp: String
          _tmp = _stmt.getText(_columnIndexOfStorageType)
          _tmpStorageType = __converters.toStorageType(_tmp)
  ...
  public override suspend fun getAllActivities(): List<ActivityEntity> {
    val _sql: String = "SELECT * FROM activities WHERE isDeleted = 0 ORDER BY createdAt DESC"
  ...
  public override suspend fun getActivityById(id: String): ActivityEntity? {
    val _sql: String = "SELECT * FROM activities WHERE id = ? AND isDeleted = 0"
    return performSuspending(__db, true, false) { _connection ->
      val _stmt: SQLiteStatement = _connection.prepare(_sql)
      try {
        var _argIndex: Int = 1
  ...
  public override suspend fun deleteActivity(id: String) {
